...
function ide
    ...
    switch $argv[1]
        ...
        case ai
            set -l ai_session_name "$session_name-ai"
            if contains $ai_session_name $sessions
                zellij attach $ai_session_name
            else
                zellij --new-session-with-layout ai --session $ai_session_name
            end

        case ""
            set -l ide_session_name "$session_name-ide"
            if contains $ide_session_name $sessions
                zellij attach $ide_session_name
            else
                zellij --new-session-with-layout ide --session $ide_session_name
            end

        case "*"
            echo "Usage: ide [mini|ai]"
            echo "  mini: Start a minimal zellij session"
            echo "  ai: Start a zellij session with AI tools"
            echo ""
